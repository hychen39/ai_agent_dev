...
提示工程透過設計與調整提示詞，引導模型完成任務。撰寫時先交代**任務、情境資訊、約束條件與輸出格式**。在 AI Agent 中，提示詞也用來定義角色、目標，以及使用工具、記憶和與使用者互動的方式。

### 2. 依任務需求選擇提示技巧

| 技巧 | 核心做法 | 本章中的用途 |
| --- | --- | --- |
| 零樣本提示（Zero-shot） | 描述任務，不提供示範範例。 | 擷取姓名與電子郵件等指令明確的任務。 |
| 思維鏈提示（Chain-of-Thought） | 引導模型將問題拆成中間步驟，再得出答案。 | 處理配送距離等需要多步計算與比較的問題。 |
| 靜態少樣本提示（Static Few-shot） | 每次使用相同的輸入與輸出範例。 | 示範固定的客戶訊息分類標準與回應格式。 |
| 動態少樣本提示（Dynamic Few-shot） | 由應用程式依當前輸入選取相關範例，再組成提示詞。 | 為配送訊息搭配相關配送案例。 |

少樣本提示讓模型參考上下文中的範例，**不會更新模型參數**。範例應正確、具代表性且格式一致，並控制數量與長度，以節省 token。

### 3. 分清楚訊息的角色、內容與中繼資料

- **角色（Role）**：System Message 定義行為與規則；Human Message 提供使用者輸入；AI Message 保存模型回應；Tool Message 保存工具執行結果。
- **內容（Content）**：訊息承載的文字、圖片或其他資料。
- **中繼資料（Metadata）**：用於辨識、追蹤與分析訊息，例如訊息 ID、模型資訊與 token 用量。
...
在提示詞中要求 JSON，可以表達格式需求，但回應仍可能缺少欄位或不符合格式。本章使用 Pydantic model 定義輸出 schema，再透過 `with_structured_output()` 取得結構化結果。

以客戶訊息分類為例：

- `category` 使用 `Literal` 定義允許值：`Billing`、`Delivery`、`Product`。
- `reason` 保存簡短的分類理由；`Field(description=...)` 說明欄位用途。
